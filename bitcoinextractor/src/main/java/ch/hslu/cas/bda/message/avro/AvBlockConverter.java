...
public class AvBlockConverter {
    ...
    public static AvBlock toAvBlock(Block block, long blockNo) {
        ...
        avBlock.setTransactions(block.getTransactions().
                stream().map(AvBlockConverter::toAvTransaction).collect(Collectors.toList()));
    ...
    private static AvTransaction toAvTransaction(Transaction tx) {
        ...
        avTx.setVin(tx.getInputs().
                stream().map(AvBlockConverter::toInput).filter(t -> t != null).collect(Collectors.toList()));

        avTx.setVout(tx.getOutputs().
                stream().map(AvBlockConverter::toOutput).filter(t -> t != null).collect(Collectors.toList()));
